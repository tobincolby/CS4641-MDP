...
public class AnalysisRunner {
	...
	public void runValueIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			//ValueIteration(Domain domain, RewardFunction rf, TerminalFunction tf, double gamma, HashableStateFactory hashingFactory, double maxDelta, int maxIterations) {
	...
	public void runPolicyIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			pi = new PolicyIteration(
					...
					hashingFactory,
					-1, 1, numIterations);
			//PolicyIteration(Domain domain, RewardFunction rf, TerminalFunction tf, double gamma, HashableStateFactory hashingFactory, double maxDelta, int maxEvaluationIterations, int maxPolicyIterations) {
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runQLearning(BasicGridWorld gen, Domain domain,
			...
			SimulatedEnvironment env, boolean showPolicyMap, int first, int second) {
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			agent = new QLearning(
				domain,
				0.99, // gamma - 0.99 for long term reward
				hashingFactory,
				0.99,
					0.10); // learning rate - 0.10 often used in practice

			if (first == 0) {
				if (second == 0) {
					agent.setLearningPolicy(new EpsilonGreedy(agent, 0.1D));
				} else if (second == 1) {
					agent.setLearningPolicy(new EpsilonGreedy(agent, 0.4D));
				} else if (second == 2) {
					agent.setLearningPolicy(new EpsilonGreedy(agent, 0.7D));
				}
			} else if (first == 1) {
				agent.setLearningPolicy(new GreedyDeterministicQPolicy(agent));
			} else if (first == 2) {
				if (second == 0) {
					agent.setLearningPolicy(new BoltzmannQPolicy(agent, 0.1));
				} else if (second == 1) {
					agent.setLearningPolicy(new BoltzmannQPolicy(agent, 0.4));
				} else if (second == 2) {
					agent.setLearningPolicy(new BoltzmannQPolicy(agent, 0.7));
				}
			}
			//agent.setLearningPolicy(new EpsilonGreedy(agent, 0.7D)); // epsilon 0.1 0.4 0.7
			//agent.setLearningPolicy(new GreedyDeterministicQPolicy(agent));
			//agent.setLearningPolicy(new BoltzmannQPolicy(agent, 0.7)); // temperature 0.1 0.4 0.7
			//QLearning(Domain domain, double gamma, HashableStateFactory hashingFactory, double qInit, double learningRate)
			// probably need to change policy (exploration strategy)
		...
		//visualize the value function and policy.
		if(showPolicyMap){
			simpleValueFunctionVis((ValueFunction)agent, p, initialState, domain, hashingFactory, "Q-Learning" + first + " " + second);
		}
